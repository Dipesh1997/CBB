...
object GoogleSheetsHelper {
    ...
    fun setupSheets(sheets: Sheets, spreadsheetId: String) {
        try {
            require(spreadsheetId.length > 5) { "FATAL: Spreadsheet ID is invalid or empty!" }
            android.util.Log.d("GoogleSheets", "Repairing/Setting up ID: [${spreadsheetId.take(10)}...]")
            ...
            if (cause is com.google.android.gms.auth.UserRecoverableAuthException) {
                throw com.google.api.client.googleapis.extensions.android.gms.auth.UserRecoverableAuthIOException(cause)
            }
            throw e
    ...
    private fun writeHeaders(sheets: Sheets, spreadsheetId: String) {
        val customersHeader = listOf("ID", "Name", "Phone", "Address", "Balance", "IsBadDebt", "CreatedBy", "LastUpdated", "ServerID", "ProfileDriveFileID")
